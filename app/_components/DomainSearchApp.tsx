...
export function DomainSearchApp() {
  ...
  const [selectedProviders, setSelectedProviders] =
    useState<DomainProvider[]>(defaultProviders.filter((p) => p !== "namecheap"))
      ...
      <section className="rounded-3xl border border-[color:var(--border-strong)] bg-white p-6 shadow-md shadow-slate-100/50">
        ...
        <div className="mt-6 flex flex-col md:flex-row md:items-center md:justify-between gap-6 border-t border-[color:var(--border-soft)] pt-6">
          <div className="space-y-2">
            ...
            {selectedProviders.includes("namecheap") && (
              <div className="mt-3 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-amber-700 font-semibold font-mono shadow-sm shadow-amber-500/5">
                ...
                <span>Warning: Namecheap checks are severely rate-limited on the current proxy plan.</span>
              </div>
            )}
          </div>
